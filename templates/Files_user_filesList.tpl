...
<div class="files_container">
    ...
    <div class="actionIcons z-menuitem-title">
        ...
        {if $publicFolder}
        <a class="fi_image fi_public" href="{modurl modname="Files" func="setAsPublic" folder=$folderName|replace:'/':'|' not=1}">{gt text="Set as not public folder"}</a>
        {elseif $folderName neq ''}
        <a class="fi_image fi_notpublic" href="{modurl modname="Files" func="setAsPublic" folder=$folderName|replace:'/':'|'}">{gt text="Set as a public folder"}</a>
        {/if}
    </div>
    ...
    <div id="actionForm" class="actionForm">
        <div class="diskSpace">
            ...
            <div style="width:{$usedSpace.widthUsage}px; background:url({getbaseurl}modules/Files/images/usage.gif);">&nbsp;</div>
    ...
    <form class="z-form" method="post" action="{modurl modname="Files" func="actionSelect" folder=$folderName|replace:'/':'|'}" id="form1">
        <table class="z-datatable" summary="table files">
            ...
            <tbody>
                ...
                <tr class="{cycle values="z-odd,z-even"}">
                    <td>&nbsp;</td>
                    <td><a class="fi_image fi_folder" href="{modurl modname="Files" func="main" folder=""}">.</a></td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                </tr>
                <tr class="{cycle values="z-odd,z-even"}">
                    <td>&nbsp;</td>
                    <td><a class="fi_image fi_folder" href="{modurl modname="Files" func="main" folder=$folderPrev|replace:'/':'|'}">..</a></td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                </tr>
                ...
                <tr class="{cycle values="z-odd,z-even"}">
                    ...
                    <td align="left">
                        {if $folderName eq ''}
                        <a class="fi_image fi_folder" href="{modurl modname="Files" func="main" folder=$file.name|replace:'/':'|'}">{$file.name}</a>
                        {else}
                        <a class="fi_image fi_folder" href="{modurl modname="Files" func="main" folder=$folderName|cat:'|'|cat:$file.name|replace:'/':'|'}">{$file.name}</a>
                        {/if}
                    </td>
                    <td>&nbsp;</td>
                    <td align="right">{$file.time|dateformat:'datetimebrief'}</td>
                    <td align="right">
                        <a href="{modurl modname="Files" func="action" do="rename" fileName=$file.name folder=$folderName|replace:'/':'|'}">{img modname='core' src='edit.gif' set='icons/extrasmall' __alt="Rename file" __title="Rename folder"}</a>
                        <a href="{modurl modname="Files" func="action" do="delete" fileName=$file.name folder=$folderName|replace:'/':'|'}">{img modname='core' src='14_layer_deletelayer.gif' set='icons/extrasmall' __alt="Delete folder" __title="Delete File"}</a>
                    </td>
                </tr>
                {/if}
                {/foreach}
                ...
                <tr class="{cycle values="z-odd,z-even"}">
                    ...
                    <td align="left">
                        <a class="fi_image" style="background: url({getbaseurl}modules/Files/images/fileIcons/{$file.fileIcon}) no-repeat 0 50%;" href="{modurl modname="Files" func="downloadFile" folder=$folderName|replace:'/':'|' fileName=$file.name|replace:'/':'|'}">{$file.name}</a>
                    </td>
