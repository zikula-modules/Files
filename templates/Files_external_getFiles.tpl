...
<div class="files_container">
    ...
    <div class="actionIcons z-menuitem-title">
        ...
        {if $publicFolder}
        <a class="fi_image fi_public" href="{modurl modname="Files" func="setAsPublic" external="1" folder=$folderName|replace:'/':'|' hook=$hook not=1}">{gt text="Set as not public folder"}</a>
        {elseif $folderName neq ''}
        <a class="fi_image fi_notpublic" href="{modurl modname="Files" func="setAsPublic" external="1" folder=$folderName|replace:'/':'|' hook=$hook}">{gt text="Set as a public folder"}</a>
        {/if}
    </div>
    ...
    <p class="z-informationmsg">
        ...
        <strong>{getbaseurl}file.php?file={$folderPath}{if $folderPath|substr:-1 neq '/'}/{/if}{gt text="file_name"}</strong>
    </p>
    {/if}

    <form class="z-form" method="post" action="{modurl modname="Files" func="actionSelect" folder=$folderName|replace:'/':'|' hook=$hook}"  id="form1">
        <div>
            ...
            <table class="z-datatable" summary="table files">
                ...
                <tbody>
                    ...
                    <tr class="{cycle values="z-odd,z-even"}">
                        <td>&nbsp;</td>
                        <td><a class="fi_image fi_folder" href="{modurl modname="Files" type="external" func="getFiles" hook=$hook folder=""}">.</a></td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr class="{cycle values="z-odd,z-even"}">
                        <td>&nbsp;</td>
                        <td><a class="fi_image fi_folder" href="{modurl modname="Files" type="external" func="getFiles" hook=$hook folder=$folderPrev}">..</a></td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                    </tr>
                    ...
                    <tr class="{cycle values="z-odd,z-even"}">
                        ...
                        <td align="left">
                            {if $folderName eq ''}
                            <a class="fi_image fi_folder" href="{modurl modname="Files" func="getFiles" type="external" hook=$hook folder=$file.name}">{$file.name}</a>
                            {else}
                            <a class="fi_image fi_folder" href="{modurl modname="Files" func="getFiles" type="external" hook=$hook folder=$folderName|cat:'/'|cat:$file.name}">{$file.name}</a>
                            {/if}
                        </td>
                        <td>&nbsp;</td>
                        <td align="right">{$file.time|dateformat:'datetimebrief'}</td>
                        <td align="right">
                            <a href="{modurl modname="Files" external="1" func="action" do="rename" fileName=$file.name folder=$folderName|replace:'/':'|' hook=$hook}">{img modname='core' src='edit.gif' set='icons/extrasmall' __alt="Rename file" __title="Rename folder"}</a>
                            <a href="{modurl modname="Files" external="1" func="action" do="delete" fileName=$file.name folder=$folderName|replace:'/':'|' hook=$hook}">{img modname='core' src='14_layer_deletelayer.gif' set='icons/extrasmall' __alt="Delete folder" __title="Delete folder"}</a>
                        </td>
                    </tr>
                    {/if}
                    {/foreach}
                    ...
                    <tr class="{cycle values="z-odd,z-even"}">
                        ...
                        <td align="left">
                            {if $publicFolder}
								{if $hook eq 1}
                            		<a class="fi_image" style="background: url({getbaseurl}modules/Files/images/fileIcons/{$file.fileIcon}) no-repeat 0 50%;" href="{modurl modname="Files" func="downloadFile" folder=$folderName|replace:'/':'|' fileName=$file.name hook=$hook}" />{$file.name}</a>
								{else}
									<a class="fi_image" style="background: url({getbaseurl}modules/Files/images/fileIcons/{$file.fileIcon}) no-repeat 0 50%;" />{$file.name}</a>
								{/if}
                            {else}
                            {$file.name}
                            {/if}
